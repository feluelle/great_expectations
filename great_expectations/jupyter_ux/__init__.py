...
def show_available_data_asset_names(context, data_source_name=None):
    ...
    for datasource in datasources:
        ...
        generators = ds.list_batch_kwarg_generators()
        for generator_info in generators:
            html += "batch_kwarg_generator: {0:s} ({1:s})".format(generator_info['name'], generator_info['class_name'])
            generator = ds.get_batch_kwarg_generator(generator_info['name'])
...
<p>Read about how batch kwarg generator derive data assets from data sources:
