...
class Datasource(object):
    ...
    For example, a batch kwarg generator could produce a SQL query that logically represents "rows in the Events
    ...
    and/or batch kwarg generators for a more generic datasource.
    ...
    def build_configuration(
            ...
            batch_kwarg_generators=None,
            **kwargs
    ):
        """
        Build a full configuration object for a datasource, potentially including batch kwarg generators with defaults.
        ...
        Args:
            ...
            batch_kwarg_generators: BatchKwargGenerators configuration dictionary
        ...
        configuration = class_.build_configuration(data_asset_type=data_asset_type, batch_kwarg_generators=batch_kwarg_generators, **kwargs)
        return configuration

    def __init__(self, name, data_context=None, data_asset_type=None, batch_kwarg_generators=None, **kwargs):
        """
        ...
        Args:
            ...
            batch_kwarg_generators: BatchKwargGenerators to add to the datasource
        """
        ...
        self._batch_kwarg_generators = {}

        self._datasource_config["data_asset_type"] = data_asset_type
        if batch_kwarg_generators is not None:
            self._datasource_config["batch_kwarg_generators"] = batch_kwarg_generators
    ...
    def _build_generators(self):
        """
        Build batch kwarg generator objects from the datasource configuration.

        Returns:
            None
        """
        try:
            for generator in self._datasource_config["batch_kwarg_generators"].keys():
                self.get_batch_kwarg_generator(generator)
        except KeyError:
            pass

    def add_batch_kwarg_generator(self, name, class_name, **kwargs):
        ...
        generator = self._build_batch_kwarg_generator(**kwargs)
        if "batch_kwarg_generators" not in self._datasource_config:
            self._datasource_config["batch_kwarg_generators"] = dict()
        self._datasource_config["batch_kwarg_generators"][name] = kwargs

        return generator

    def _build_batch_kwarg_generator(self, **kwargs):
        ...
        generator = instantiate_class_from_config(
            ...
            config_defaults={
                "module_name": "great_expectations.datasource.batch_kwarg_generator"
            }
        )
        return generator

    def get_batch_kwarg_generator(self, name):
        ...
        Returns:
            BatchKwargGenerator (BatchKwargGenerator)
        """
        if name in self._batch_kwarg_generators:
            return self._batch_kwarg_generators[name]
        elif "batch_kwarg_generators" in self._datasource_config and name in self._datasource_config["batch_kwarg_generators"]:
            generator_config = copy.deepcopy(self._datasource_config["batch_kwarg_generators"][name])
        else:
            raise ValueError(
                "Unable to load batch kwarg generator %s -- no configuration found or invalid configuration." % name
            )
        generator = self._build_batch_kwarg_generator(**generator_config)
        self._batch_kwarg_generators[name] = generator
        return generator

    def list_batch_kwarg_generators(self):
        ...
        if "batch_kwarg_generators" in self._datasource_config:
            for key, value in self._datasource_config["batch_kwarg_generators"].items():
                generators.append({
                    "name": key,
    ...
    def get_available_data_asset_names(self, batch_kwarg_generator_names=None):
        """
        Returns a dictionary of data_asset_names that the specified batch kwarg
        generator can provide. Note that some batch kwarg generators may not be
        capable of describing specific named data assets, and some (such as
        filesystem glob batch kwarg generators) require the user to configure
        data asset names.

        Args:
            batch_kwarg_generator_names: the BatchKwargGenerator for which to get available data asset names.
        ...
        if batch_kwarg_generator_names is None:
            batch_kwarg_generator_names = [generator["name"] for generator in self.list_batch_kwarg_generators()]
        elif isinstance(batch_kwarg_generator_names, string_types):
            batch_kwarg_generator_names = [batch_kwarg_generator_names]

        for generator_name in batch_kwarg_generator_names:
            generator = self.get_batch_kwarg_generator(generator_name)
            available_data_asset_names[generator_name] = generator.get_available_data_asset_names()
        return available_data_asset_names

    def build_batch_kwargs(self, batch_kwarg_generator, name=None, partition_id=None, **kwargs):
        generator_obj = self.get_batch_kwarg_generator(batch_kwarg_generator)
