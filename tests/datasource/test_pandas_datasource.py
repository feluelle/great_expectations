...
def test_standalone_pandas_datasource(test_folder_connection_path):
    datasource = PandasDatasource('PandasCSV', batch_kwarg_generators={
            "subdir_reader": {
    ...
    generator = datasource.get_batch_kwarg_generator("subdir_reader")
...
def test_create_pandas_datasource(data_context, tmp_path_factory):
    ...
    data_context.add_datasource(name, class_name=class_name, batch_kwarg_generators={
            "subdir_reader": {
                "class_name": "SubdirReaderBatchKwargsGenerator",
    ...
    assert data_context_file_config["datasources"][name]["batch_kwarg_generators"]["subdir_reader"]["class_name"] == \
...
def test_pandas_datasource_custom_data_asset(data_context, test_folder_connection_path):
    ...
    batch = data_context.get_batch(expectation_suite_name="test",
                                   batch_kwargs=data_context.build_batch_kwargs(datasource=name,
                                                                                batch_kwarg_generator="subdir_reader", name="test")
    )
...
def test_invalid_reader_pandas_datasource(tmp_path_factory):
    ...
    datasource = PandasDatasource('mypandassource', batch_kwarg_generators={
            "subdir_reader": {
                "class_name": "SubdirReaderBatchKwargsGenerator",
